...
var Tick = function Tick(_ref) {
  ...
  }), index % 10 === 0 && React__default.createElement(reactNative.Text, {
...
var Slider = function Slider(_ref3) {
  ...
  var TICKS_ARRAY = lodashEs.range(min, max, step);
  var NUMBER_OF_TICKS = TICKS_ARRAY.length;

  var _useState = React.useState(FULL_WIDTH),
      width = _useState[0],
  ...
  var _useState2 = React.useState(function () {
    return new reactNative.Animated.Value(valueToPx(value));
  }),
  ...
  var onHandlerStateChange = function onHandlerStateChange(event) {
    ...
    if (event.nativeEvent.state === reactNativeGestureHandler.State.END) {
      activeValue = dragXToValue(valueToPx(value) + offsetX.current + event.nativeEvent.translationX);
    }

    if (event.nativeEvent.oldState === reactNativeGestureHandler.State.ACTIVE) {
      offsetX.current += event.nativeEvent.translationX;
      onChange(lodashEs.clamp(activeValue, min, max));
    }
  };

  var onGestureEvent = function onGestureEvent(event) {
    if (event.nativeEvent.state === reactNativeGestureHandler.State.ACTIVE || event.nativeEvent.state === reactNativeGestureHandler.State.END) {
  ...
  return React__default.createElement(reactNativeGestureHandler.PanGestureHandler, {
...
exports.Tick = Tick;
//# sourceMappingURL=react-native-scrub.cjs.development.js.map
